...
func (h *Handler) adminLogin(c echo.Context) error {
	...
	if res, err := tx.Exec(query, sess.UserID, sess.SessionID, sess.CreatedAt, sess.UpdatedAt, sess.ExpiredAt); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	} else {
...
func (h *Handler) adminBanUser(c echo.Context) error {
	...
	if _, err = h.DBU(userID).Exec(query, userID, requestAt, requestAt, requestAt); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
