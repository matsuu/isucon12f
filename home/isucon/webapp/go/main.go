...
func (h *Handler) checkOneTimeToken(token string, tokenType int, requestAt int64) error {
	ctx := context.TODO()
	ex, err := h.Redis.GetDel(ctx, token).Result()
	if err != nil {
		if err == redis.Nil {
	...
	expiredAt, err := strconv.ParseInt(ex, 10, 64)
	if err != nil {
		return err
	}
	if expiredAt < requestAt {
...
func (h *Handler) checkViewerID(userID int64, viewerID string) error {
	...
	if err := h.DBU(userID).Get(device, query, userID, viewerID); err != nil {
...
func (h *Handler) checkBan(userID int64) (bool, error) {
	...
	if err := h.DBU(userID).Get(banUser, query, userID); err != nil {
		if err == sql.ErrNoRows {
			return false, nil
		}
		return false, err
...
func (h *Handler) loginProcess(tx *sqlx.Tx, userID int64, requestAt int64) (*User, []*UserLoginBonus, []*UserPresent, error) {
	...
	// ログインボーナス処理
	bonusCoin, bonusCards, bonusItems, loginBonuses, err := h.obtainLoginBonus(tx, userID, requestAt)
	if err != nil {
		return nil, nil, nil, err
	}
	...
	query = "UPDATE users SET updated_at=?, last_activated_at=?, isu_coin=isu_coin+? WHERE id=?"
	if _, err := tx.Exec(query, requestAt, requestAt, bonusCoin, userID); err != nil {
		return nil, nil, nil, err
	}
...
func (h *Handler) obtainLoginBonus(tx *sqlx.Tx, userID int64, requestAt int64) (int64, []*UserCard, []*UserItem, []*UserLoginBonus, error) {
	...
	for _, bonus := range loginBonuses {
		...
		if err := tx.Get(rewardItem, query, bonus.ID, userBonus.LastRewardSequence); err != nil {
			if err == sql.ErrNoRows {
				return 0, nil, nil, nil, ErrLoginBonusRewardNotFound
			}
			return 0, nil, nil, nil, err
		}
		...
		default:
			return 0, nil, nil, nil, ErrInvalidItemType
		}

		// 進捗の保存
		if initBonus {
			query = "INSERT INTO user_login_bonuses(user_id, login_bonus_id, last_reward_sequence, loop_count, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)"
			if res, err := tx.Exec(query, userBonus.UserID, userBonus.LoginBonusID, userBonus.LastRewardSequence, userBonus.LoopCount, userBonus.CreatedAt, userBonus.UpdatedAt); err != nil {
				return 0, nil, nil, nil, err
			} else {
	...
	return obtainCoin, obtainCards, obtainItems, sendLoginBonuses, nil
}
...
func initialize(c echo.Context) error {
	...
	for _, host := range dbHosts {
		...
		go func(host string) {
			defer wg.Done()
			out, err := exec.Command("/bin/sh", "-c", SQLDirectory+"init2.sh "+host).CombinedOutput()
			if err != nil {
...
func (h *Handler) login(c echo.Context) error {
	...
	user := new(User)
	query := "SELECT * FROM users WHERE id=?"
	if err := h.DBU(req.UserID).Get(user, query, req.UserID); err != nil {
		if err == sql.ErrNoRows {
...
func (h *Handler) listGacha(c echo.Context) error {
	...
	ctx := context.TODO()
	if err := h.Redis.Set(ctx, token.Token, token.ExpiredAt, 0).Err(); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
...
func (h *Handler) listPresent(c echo.Context) error {
	...
	LIMIT ? OFFSET ?`
	if err = h.DBU(userID).Select(&presentList, query, userID, PresentCountPerPage, offset); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}

	var presentCount int
	if err = h.DBU(userID).Get(&presentCount, "SELECT COUNT(*) FROM user_presents WHERE user_id = ? AND deleted_at IS NULL", userID); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
...
func (h *Handler) receivePresent(c echo.Context) error {
	...
	if err = h.DBU(userID).Select(&obtainPresent, query, params...); err != nil {
		return errorResponse(c, http.StatusBadRequest, err)
	}
	...
	if obtainCoin > 0 {
		...
		if _, err := tx.Exec(query, obtainCoin, userID); err != nil {
			return errorResponse(c, http.StatusInternalServerError, err)
		}
	}
...
func (h *Handler) listItem(c echo.Context) error {
	...
	if err = h.DBU(userID).Select(&itemList, query, userID); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}

	cardList := make([]*UserCard, 0)
	query = "SELECT * FROM user_cards WHERE user_id=?"
	if err = h.DBU(userID).Select(&cardList, query, userID); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
...
func (h *Handler) addExpToCard(c echo.Context) error {
	...
	`
	if err = h.DBU(userID).Get(card, query, cardID, userID); err != nil {
		if err == sql.ErrNoRows {
			return errorResponse(c, http.StatusNotFound, err)
		}
		return errorResponse(c, http.StatusInternalServerError, err)
	...
	for _, v := range req.Items {
		...
		if err = h.DBU(userID).Get(item, query, v.ID, userID); err != nil {
			if err == sql.ErrNoRows {
				return errorResponse(c, http.StatusNotFound, err)
			}
			return errorResponse(c, http.StatusInternalServerError, err)
...
func (h *Handler) updateDeck(c echo.Context) error {
	...
	if err = h.DBU(userID).Select(&cards, query, params...); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
...
func (h *Handler) reward(c echo.Context) error {
	...
	if err = h.DBU(userID).Get(deck, query, userID); err != nil {
		if err == sql.ErrNoRows {
			return errorResponse(c, http.StatusNotFound, err)
		}
		return errorResponse(c, http.StatusInternalServerError, err)
	...
	cards := make([]*UserCard, 0)
	query = "SELECT * FROM user_cards WHERE id IN (?, ?, ?)"
	if err = h.DBU(userID).Select(&cards, query, deck.CardID1, deck.CardID2, deck.CardID3); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
	if len(cards) != 3 {
	...
	if _, err = h.DBU(userID).Exec(query, user.IsuCoin, user.LastGetRewardAt, user.ID); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
